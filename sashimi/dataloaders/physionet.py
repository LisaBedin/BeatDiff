...
class PhysionetNPZ(Dataset):
    def __init__(self, database_path: str, training_class='Train', normalized: str = 'no', segment_length=512,
                 return_beat_id: bool = False, all_limb=False, noise_path='',
                 **kwargs):

        all_files = np.array(glob.glob(os.path.join(database_path, '*/*.npz')))
        kept_files = []
        all_mean, all_rr, all_age, all_sex = [], [], [], []
        for file_p in tqdm(all_files, total=len(all_files), desc='filtering empty ecg'):
            npz = np.load(file_p)
            ecg = npz['data']
            if ecg.shape[1] >= 1:
                try:
        ...
        ind_val, ind_test = train_test_split(ind_val, test_size=0.5, random_state=0)
        ind_train, ind_val = np.array(ind_train, dtype=int), np.array(ind_val, dtype=int)
        ind_test = np.array(ind_test, dtype=int)
        print(f'train = {len(ind_train)} | val = {len(ind_val)} | test = {len(ind_test)}')

        if 'Train' in training_class:
            self.file_paths = kept_files[ind_train]
        elif 'Val' in training_class or 'CV' in training_class:
            self.file_paths = kept_files[ind_val]
        else:
            self.file_paths = kept_files[ind_test]
        self.start, self.end = 200, 712
        assert self.end - self.start == segment_length
...
class PhysionetECG(Dataset):
    ...
    def __init__(self, database_path: str, categories_to_filter: List[str], segment_length: int = 176,
                 estimate_noise_std: bool = False, normalized: str = 'no', training_class: bool = 'Training',
        ...
        all_means = []
        all_shapes = []
        for row in tqdm(ids, total=len(ids), desc='filtering empty ecg'):
            file_p = os.path.join(database_path, row.dataset_name, f'{row.dataset_id}.npz')
            npz = np.load(file_p)
            ecg = npz['data']
            if not np.isnan(ecg.mean()):
                all_means.append(ecg.mean())
                all_shapes.append(ecg.shape)
                kept_ids.append(row)
        self._ids = kept_ids
