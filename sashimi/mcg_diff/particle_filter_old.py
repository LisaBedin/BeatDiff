...
def predict(score_model: ScoreModel,
    ...
    diffusion_steps = (t * torch.ones((particles.shape[0], 1))).cuda()  # use the corresponding reverse step
    if hasattr(score_model.net, 'device_ids'):
        ...
        for batch_idx in range(n_batches):
            epsilon_predicted.append(score_model.net(particles[batch_size*batch_idx:(batch_idx+1)*batch_size],
                                                     diffusion_steps[batch_size*batch_idx:(batch_idx+1)*batch_size]).cpu())
        epsilon_predicted = torch.cat(epsilon_predicted, dim=0).to(particles.device)
    else:
        epsilon_predicted = score_model.net(particles, diffusion_steps).to(particles.device)
