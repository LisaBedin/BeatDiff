...
def main(cfg: DictConfig) -> list:
    ...
    for (batch_test_ecg, batch_test_features) in test_dataloader:
        ...
        ref_samples = posterior_sampling_fun(random.split(random.PRNGKey(key), n_patients),
                                             observations,
                                             variances,
                                             batch_test_features[:n_patients])
        np.savez('/mnt/data/gabriel/ecg_inpainting/models/ecg_template/template_ecg_conditional/baseline/qt_conditioning.npz',#os.path.join(results_path, 'qt_as_rr.npz'),
        ...
        for i, real_rr in enumerate(jnp.linspace(600, 1200, num=20)):
            ...
            for j, (patient_samples, ref) in enumerate(zip(ref_samples, batch_test_ecg[:n_patients])):
                ...
                fig.savefig(f'images/example_{j}_QT_as_RR_{int(real_rr)}.pdf')
                plt.close(fig)
        ...
        # np.savez('/mnt/data/gabriel/ecg_inpainting/models/ecg_template/template_ecg_conditional/baseline/qt_as_rr.npz',#os.path.join(results_path, 'qt_as_rr.npz'),
        #          qts_mean=np.concatenate(qts_mean),
        #          qts_interval=np.concatenate(qts_interval),
        #          rrs=np.concatenate(rrs))
