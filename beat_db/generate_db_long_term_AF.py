...
def get_beat_ecg_from_headers(x, Dx_map):
    ...
    if len(lab) > 0:
        ...
        age = get_age(header)
        sex = get_sex(header)
        return ecg, id_in_dataset, lab, sex, age
    return None
...
def get_labels_from_header(header, Dx_map):
    ...
    class_labels = []
    for label in labels:
        if int(label) == 164889003:
            class_labels.append('AF')
        if int(label) == 164890007:
            class_labels.append('AFL')
...
if __name__ == '__main__':
    ...
    data_destination = '/mnt/data/lisa/physionet.org/AF'  # '/mnt/data/lisa/physionet.org/ecg_10s_db'  # sys.argv[2]
