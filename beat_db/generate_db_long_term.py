...
def get_beat_ecg_from_headers(x, Dx_map):
    ...
    age = get_age(header)
    sex = get_sex(header)
    return ecg, id_in_dataset, get_labels_from_header(header, Dx_map), sex, age
...
def get_beats_from_ecg(ecg, method='pca', n_components=3):
    ...
    r_peaks = r_peaks[is_r_peak_valid]
    beats = ecg[:, [np.arange(peak - 48, peak + 128) for peak in r_peaks]]
    rr = np.array([r1 - r2 if r1 - r2 < 500 else None for r1, r2 in zip(r_peaks[1:], r_peaks[:-1])])
    beats = beats[:, 1:]
    return beats, rr
...
def get_labels_from_header(header, Dx_map):
    ...
    for label in labels:
        try:
            ...
            try:
                current_class_label = Dx_map.loc[Dx_map['Snomed_CT'] == int(label), 'Acronym Name'].iloc[0]
            except:
...
def generate_data_for_db(inc, path):
    ...
    try:
        beats, id_in_dataset, labels, sex, age = get_beat_ecg_from_headers((header, recording_file), Dx_map=Dx_map)
...
if __name__ == '__main__':
    data_source = '/mnt/data/lisa/physionet.org/files/ecg-arrhythmia/1.0.0/WFDBRecords'  #'/mnt/data/gabriel/physionet.org/files/challenge-2021/1.0.3/training'  # sys.argv[1]
    data_destination = '/mnt/data/lisa/physionet.org/arrhythmia'  # '/mnt/data/lisa/physionet.org/ecg_10s_db'  # sys.argv[2]
    n_procs = 40  # int(sys.argv[3])
    ...
    n_batches = 1000
    if 'arrhythmia' in data_source:
        Dx_map_path = '/'.join(data_source.split('/')[:-1])
        Dx_map = pd.read_csv(os.path.join(Dx_map_path, 'ConditionNames_SNOMED-CT.csv'))
    else:
        if os.path.exists('Dx_map_physionet.csv'):
        ...
        for database in available_databases:
            ...
            for folder in subfolders:
                if n_procs > 0:
                    ...
                    list_f = find_challenge_files(os.path.join(database_path, folder))
                    batch_meta = [generate_data_fun(f) for f in tqdm(list_f, total=len(list_f))]
